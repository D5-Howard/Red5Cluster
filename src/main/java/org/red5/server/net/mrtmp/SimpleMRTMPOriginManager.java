...
public class SimpleMRTMPOriginManager implements IMRTMPOriginManager {
	...
	public boolean registerConnection(IMRTMPConnection conn) {
		//log.info("[D5Power] Register RTMPOriginConnection " + conn + "=============================="+((MRTMPOriginConnection) conn).getIoSession().getId());
		lock.writeLock().lock();
		try {
	...
	public IMRTMPConnection lookupMRTMPConnection(RTMPConnection rtmpConn) {
		...
		try {
			...
			if (conn == null && !rtmpConn.getType().toLowerCase().equals(IConnection.Type.PERSISTENT.name().toLowerCase())) {
				if (connSet.size() > 0) {
